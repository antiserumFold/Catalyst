    ...
    uint8_t            generation8)
{
    ...
    // Preserve the existing move if the caller doesn't have one,
    // but only when we're updating the same position.
    if (newMove != MOVE_NONE || newKey16 != this->key16)
        this->move = newMove;

    // Replacement conditions (cheapest checks first):
    //   - exact bound always wins
    //   - different position: overwrite unconditionally
    //   - entry is stale (from a previous search)
    //   - new search is deep enough to be worth replacing
    if (newFlag == TT_EXACT || newKey16 != this->key16 || relative_age(generation8) != 0
        || newDepth + 4 + int(isPv) * 2 > get_depth())
    {
        this->key16  = newKey16;
        this->depth8 = static_cast<uint8_t>(newDepth + TT_DEPTH_OFFSET);
        this->genBound8
            = generation8 | (static_cast<uint8_t>(isPv) << 2) | static_cast<uint8_t>(newFlag);
...
// ============================================================
// TT::clear — zeroes the table using multiple threads
// ============================================================
...
// ============================================================
// TT::new_search — bump the generation counter each root search
// ============================================================

void TT::new_search()
{
    // Only advance the upper 5 bits; lower 3 bits are pv+flag and must stay zero here.
...
std::tuple<bool, TTData, TTWriter> TT::probe(Key key) const
{
    ...
    // Prefer empty slots immediately; otherwise use depth minus age penalty.
    ...
    for (int i = 1; i < TTCluster::ENTRIES; ++i)
    {
        ...
        // Empty slot: grab it immediately, no need to look further.
        if (!e.is_occupied())
...
// ============================================================
// TT::hashfull — permill of entries used in the current generation
// ============================================================
...
int TT::hashfull() const
{
    ...
    // Divide by ENTRIES (not 4) since clusters now have 3 entries.
